...
<div class="container">
    <div class="panel panel-default">
        <input id="killId" value="${itemDetail.id}" type="hidden"/>
        <div class="panel-heading">
            <h1>商品名称：${itemDetail.itemName}</h1>
        </div>
        <div class="panel-body">
            <h2 class="text-danger">剩余数量：${itemDetail.total}</h2>
        </div>
        <div class="panel-body">
            <h2 class="text-danger">开始时间：<fmt:formatDate value="${itemDetail.startTime}" pattern='yyyy-MM-dd HH:mm:ss'/></h2>
        </div>
        <div class="panel-body">
            <h2 class="text-danger">结束时间：<fmt:formatDate value="${itemDetail.endTime}" pattern='yyyy-MM-dd HH:mm:ss'/></h2>
        </div>
    </div>

    <td>
        <c:choose>
            <c:when test="${itemDetail.canKill==1}">
